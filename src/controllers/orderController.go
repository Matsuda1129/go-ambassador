...
func CreateOrder(c *fiber.Ctx) error {
	...
	stripe.Key = "sk_test_51J8AgUDSN21tOOUxXxn9Dj6fpqyzgaoa5F0pUF2nKlD1c29QHK2eD0clmZbtvOD27cbw275GY7bhXXYajhXd1vGx00Y4Dq1rxY"
...
func CompleteOrder(c *fiber.Ctx) error {
	...
	go func(order models.Order) {
		...
		ambassadorMessage := []byte(fmt.Sprintf("You earned $%f from tho link #%s", ambassadorRevenue, order.Code))

		smtp.SendMail("http://0.0.0.0:8025", nil, "no-reply@email.com", []string{order.AmbassadorEmail}, ambassadorMessage)

		adminMessage := []byte(fmt.Sprintf("Order #%d with a total of $%f has been completed", order.Id, adminRevenue))

		smtp.SendMail("host.docker.internal:8025", nil, "no-reply@email.com", []string{"admin@admin.com"}, adminMessage)
